...
sudoku(Matrix) :- length(Matrix, 9), %create Matrix, a list with 9 elements 
				maplist(same_length(Matrix), Matrix), %now Matrix is a matrix of 9 lines and 9 columns 
				append(Matrix, Vars), Vars ins 1..9, %all the elements of the matrix are between 1 and 9
				maplist(all_distinct, Matrix), %all the elements in the lines have to be distinct
				transpose(Matrix, TransMatrix), %the line of Matrix became the columns
				maplist(all_distinct, TransMatrix),%all the elements in the columns (that now correspond to the lines) have to be distinct
				Matrix = [L1,L2,L3,L4,L5,L6,L7,L8,L9], %Matrix is defined by 9 lines
				L1 = [E11, E12, E13, E14, E15, E16, E17, E18, E19], %there I defined each line with 9 elements corresponding to the lines and columns position (E45 correspond to the element line 4 and columns 5
